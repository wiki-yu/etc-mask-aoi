...
def detect_nose_band(img, img_hsv, points, hue_thresh):
    p1 = points[0]
    p2 = points[1]
    p3 = points[2]
    p4 = points[3]
    ...
    img_v = cv2.bitwise_and(img_v, img_v, mask=mask1)
...
def mask_inspection(frame, img_hsv, points, hue_thresh):
    eval = 'OK'
    coord_noseband = detect_nose_band(frame, img_hsv, points, hue_thresh)
...
def main():
    ...
    out = cv2.VideoWriter('./out_test1.avi', cv2.VideoWriter_fourcc(*'XVID'), 15, (width, height))
    ...
    while True:
        ...
        if mask_new == True:
            ...
            if frame_check < frame_check_amount:
                eval = mask_inspection(frame, points)
                frame_check += 1
        ...
        cv2.imwrite("./temp2/{}.jpg".format(frame_count), frame)
...
# coord_noseband = detect_nose_band(frame, img_hsv, p1, p2, p3, p4, hue_thresh)
# if coord_noseband == None:
#     print("no noseband!")
# else:
#     width_start = coord_noseband[0]
#     width_end = coord_noseband[1]
#     height = coord_noseband[2]
#     cv2.putText(frame, "Length: {}".format(width_end - width_start), (300, 500), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
#     cv2.line(frame, (width_start, height), (width_end, height), (255, 0, 0), 3)

# coord_welding = detect_welding(frame, img_hsv, p1, p2, p3, p4, hue_thresh)
# if coord_welding == None:
#     print("no welding!")
# else:
#     width_start = coord_noseband[0]
#     width_end = coord_noseband[1]
...
# cv2.rectangle(frame, rect_p1, rect_p2, (0, 255, 0), 1)